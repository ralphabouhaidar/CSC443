...
if(!isset($_POST["first"]) || !isset($_POST["last"]) || !isset($_POST["position"]) || !isset($_POST["email"]) || !isset($_POST["address"]) || !isset($_POST["phone"]) || !isset($_POST["salary"]))
{
    echo "fields not set";
    exit();
}
...
{ 
    ...
    {
        $sql="INSERT INTO Employees(emp_email, emp_first, emp_last, password, address, emp_phone, position, Salaries) 
                VALUES (\"".$email."\", \"".$first."\", \"".$last."\", \"".$pass."\", \"".$address."\", \"".$phone."\", \"".$position."\", \"".$salary."\")";
        $result=$connection->query($sql);
        echo "Success!";
    }
